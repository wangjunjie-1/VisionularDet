# this model design for NPU
# 1. replace depthwise conv to CONV2D
# 2. change che channel num to 64
import torch
...
class WZShuffleV2Block(nn.Module):
    def __init__(self, inp, oup, stride, activation="ReLU"):
        super(WZShuffleV2Block, self).__init__()
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                nn.Conv2d(
                    inp, branch_features, kernel_size=3, stride=self.stride, padding=1,bias=True
                ),
...
class WZShuffleNetV2(nn.Module):
    def __init__(
        self,
        model_size="1.5x",
        out_stages=(2, 3, 4),
        ...
        super(WZShuffleNetV2, self).__init__()
        ...
        elif model_size == "wz64":
            self._stage_out_channels = [16, 64, 64, 64, 64]
        elif model_size == "wz128":
            self._stage_out_channels = [16, 128, 128,128,128]
        elif model_size == "wz256":
            self._stage_out_channels = [16, 128, 256, 256,256]
        elif model_size == "wz512":
            self._stage_out_channels = [16, 128, 256, 512,512]
        else:
