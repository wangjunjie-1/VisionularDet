...
def get_keep_ratio_shape(
    src_shape: Tuple[int, int],
    dst_shape: Tuple[int, int],
    ratio: Optional[float] = 9/16,
) -> Tuple[int, int]:
    """Calculate keep ratio shape"""
    src_w, src_h = src_shape
    dst_w, dst_h = dst_shape

    if src_h/src_w < ratio:
        rw = dst_w / src_w
        rh = rw/ratio
    else:
        rh = dst_h / src_h
    ...
    dst_w = int(rw * src_w)
    dst_h = int(rh * src_h)
    # print(src_shape,dst_shape,(dst_w, dst_h))
    return dst_w, dst_h
...
class ShapeTransform:
    ...
    def __call__(self, meta_data, dst_shape):
        ...
        if self.keep_scale:
            dst_shape = get_keep_ratio_shape(
